...
# RH: modified build script to bypass module loading
if [ -z "$PREFIX" ] ; then
    echo "PREFIX must be set for build-local.sh"
    exit 1
fi
...
function DoCMake () {
    ...
    cd    $root/build_$1$2
    echo "cmake -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype} ../src/cmake &>build_$1$2/cmake_$1.log"
          cmake -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype} ../src/cmake &>cmake_$1.log
    if [ $? -ne 0 ]; then
...
echo About to call CreateCMakedir
CreateCMakedir ${config} ${buildDirExtension}

echo About to call DoCMake
